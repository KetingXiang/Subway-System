...
public class SubwayManagerImpl implements SubwayManager
{
        ...
        throws SubwayException
    {
        //TODO 待考生实现
    	Card card = new Card();
    	int costMoney = calCostMoney(cardId, enterStation, enterTime, exitStation, exitTime);
    ...
    // 计算花费
    public int calCostMoney(String cardId, String enterStation, String enterTime, String exitStation, String exitTime) 
    {
    	int costMoney = 0;
    	if (enterStation.equals(exitStation))
    	{
    		CardEnum cardEnum = CardManagerImpl.allCards.get(cardId).getCardType();
    		int time = calTime(enterTime, exitTime);
    		if (time <= 30)
    		{
    			switch (cardEnum) {
				case A:
					costMoney = CardManagerImpl.allCards.get(cardId).getMoney(); 
					break;
    	...
    	{
    		int distance = calDistance(enterStation, exitStation);
    	}
    ...
    // 进出站为同一站点时，计算进出站时长
    public int calTime(String enterTime, String exitTime) 
    {
		int time = 0;
    	int enterHour = Integer.valueOf(enterTime.substring(0, 2));
		int enterMinute = Integer.valueOf(enterTime.substring(3, 5));
		int exitHour = Integer.valueOf(exitTime.substring(0, 2));
		int exitMinute = Integer.valueOf(exitTime.substring(3, 5));
		time = (exitHour - enterHour) * 60 + (exitMinute - enterMinute);
		
		return time;
	}
    
    // 计算距离
    public int calDistance(String enterStation, String exitStation) 
    {
    	int enterNum = Integer.valueOf(enterStation.substring(1));
    	int exitNum = Integer.valueOf(exitStation.substring(1));
    	if (enterNum >= exitNum)
    	{
    		int temp = enterNum;
    		enterNum = exitNum;
    	...
    	int distance = 0;
    	for (int i = enterNum; i < exitNum; i ++)
    	{
    		distance = distance + subways.getStationDistances().column("S" + String.valueOf(i))
    				.get("S" + String.valueOf(i)).getDistance();
    	}
